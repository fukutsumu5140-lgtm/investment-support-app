...
st.title("投資サポートアプリ")
st.caption("上部のタブから、トレンド確認・スクリーニング・お気に入り・保有銘柄・銘柄検索・比較を切り替えられます。")
...
tab_trend, tab_screen, tab_fav, tab_hold, tab_search, tab_compare = st.tabs(
    ["📈 トレンド", "🔍 スクリーニング", "★ お気に入り", "📌 保有銘柄", "🔎 銘柄検索", "⚖ 比較"]
)
...
with tab_trend:
    ...
    if not _github_token():
        ...
        st.caption("「★ お気に入り」「📌 保有銘柄」タブで登録すると、ここに直近の株価推移が表示されます。")
    else:
        period_options = {"1ヶ月": 30, "3ヶ月": 90, "6ヶ月": 180, "1年": 365, "2年": 730}
        period_label = st.radio(
            "表示期間", list(period_options.keys()), index=1, horizontal=True, key="trend_period"
        )
        period_days = period_options[period_label]
        st.caption("無料プランはデータが過去2年分までのため、「2年」を選ぶと取得できる期間が短くなる場合があります。")

        trend_cols = st.columns(2)
        for i, (code, info) in enumerate(trend_targets.items()):
            try:
                trend_df = fetch_price_trend(api_key, code, days=period_days)
            except Exception as e:
...
with tab_fav:
    ...
    if not _github_token():
        ...
        add_labels = st.multiselect(
            "追加する銘柄を選んでください（「🔍 スクリーニング」タブの絞り込み結果から選べます）",
            option_labels,
            key="fav_add_select",
        )
...
# ============================================================
# タブ：銘柄検索（個別銘柄の業績推移・財務健全性）
# ============================================================
with tab_search:
    st.subheader("個別銘柄の業績推移・財務健全性を見る")
    st.caption(
